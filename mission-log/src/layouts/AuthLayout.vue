...
const nameApp = import.meta.env.VITE_NAME_APP ?? 'NAME'
</script>
...
<template>
  <div>
    ...
    <div class="min-h-screen w-full flex bg-white dark:bg-zinc-950">
      ...
      <!--
        Panel decorativo. Espacio reservado para una imagen o SVG de fondo
        propio del login/register, por ejemplo:
          <img src="@/assets/auth-illustration.svg" class="absolute inset-0 h-full w-full object-cover" />
        Mientras tanto se usa un degradado con la paleta de marca.
      -->
      <div class="hidden lg:block lg:w-2/3 relative overflow-hidden bg-gradient-to-bl from-brand-pink via-brand-orange  to-brand-cyan">
        <div class="absolute inset-0 bg-black/10" />
  
        <div class="relative z-10 h-full flex flex-col justify-end p-12 text-white">
          <slot name="illustration">
            
            <p class="bg-white/50 rounded-md shadow-lg text-sm/relaxed max-w-sm text-black font-bold px-6">
              Centraliza reuniones, ideas, requerimientos y tareas en un único flujo de trabajo.
            </p>
          </slot>
        </div>
      </div>
